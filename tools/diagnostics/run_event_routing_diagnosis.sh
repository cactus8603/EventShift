...
conda run --no-capture-output -n mask2former \
  python tools/diagnose_event_routing.py \
  --rgb-config configs/Mask2Former_SwinL_CoSEC_DayNight_Finetune.yaml \
  --rgb-weights work_dirs/swinL_seqholdout305_from_latest_night/best_model_cosec_day.pth \
  ...
  --device cuda:0 \
  --out work_dirs/diagnostics/event_routing_val.json \
  "$@"
