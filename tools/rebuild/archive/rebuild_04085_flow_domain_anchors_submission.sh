...
BASE_MIN_VOTES="${BASE_MIN_VOTES:-4}"
...
DAY_ANCHOR_DIR="${DAY_ANCHOR_DIR:-${EVENT_RAW}/mask2former_day_event}"
NIGHT_ANCHOR_DIR="${NIGHT_ANCHOR_DIR:-${EVENT_RAW}/segformer_night_event}"
REAL_ANCHOR_DIR="${REAL_ANCHOR_DIR:-${FULLDESC_RAW}/mask2former_real}"

BASE_DAY_DIR="${BASE_DAY_DIR:-${PRED_ROOT}/${TAG}_base_day_min${BASE_MIN_VOTES}_dayanchor}"
BASE_NIGHT_DIR="${BASE_NIGHT_DIR:-${PRED_ROOT}/${TAG}_base_night_min${BASE_MIN_VOTES}_nightanchor}"
BASE_COMPOSED_DIR="${BASE_COMPOSED_DIR:-${COMPOSED_ROOT}/${TAG}_base_min${BASE_MIN_VOTES}_domainanchors}"
BASE_ZIP="${BASE_ZIP:-${ZIP_ROOT}/${TAG}_base_min${BASE_MIN_VOTES}_domainanchors.zip}"

FINAL_DAY_DIR="${FINAL_DAY_DIR:-${PRED_ROOT}/${TAG}_reqday_min${REQUIRED_MIN_VOTES}}"
FINAL_NIGHT_DIR="${FINAL_NIGHT_DIR:-${PRED_ROOT}/${TAG}_reqnight_min${REQUIRED_MIN_VOTES}}"
FINAL_REAL_DIR="${FINAL_REAL_DIR:-${PRED_ROOT}/${TAG}_reqreal_min${REQUIRED_MIN_VOTES}}"
...
check_inputs() {
  require_dir "${DAY_ANCHOR_DIR}"
  require_dir "${NIGHT_ANCHOR_DIR}"
  require_dir "${REAL_ANCHOR_DIR}"
  require_dir "${KFOLD_RAW}"
  require_dir "${FULLDESC_RAW}"
  require_dir "${HRNET_DAY}"
  require_dir "${HRNET_NIGHT}"
}
...
run_compare() {
  ...
  if [[ -f "${REFERENCE_BASE_ZIP}" && -f "${BASE_ZIP}" ]]; then
    run_py "${SWIN_L_ROOT}/tools/compare_submission_zips.py" \
      ...
      --candidate "${BASE_ZIP}" \
      --out "${DIFF_ROOT}/${TAG}_base_vs_original_04085_base.json"
  fi
...
main() {
  ...
  check_inputs
  echo "TAG=${TAG}"
  echo "DAY_ANCHOR_DIR=${DAY_ANCHOR_DIR}"
