...
day_log="${log_dir}/scale-calib-exp1G-day-eventsoft_${stamp}.log"
night_log="${log_dir}/scale-calib-exp1H-night-eventsoft_${stamp}.log"

screen -dmS scale_calib_exp1G_day_eventsoft bash -lc "
  cd '${ROOT}' &&
  export PYTHONNOUSERSITE=1 &&
  export PYTHONUNBUFFERED=1 &&
  CUDA_VISIBLE_DEVICES='${GPU_SCALE_CALIB_DAY}' conda run --no-capture-output -n '${CONDA_ENV}' \
    ...
    --reuse-eval-cache \
    --out-dir work_dirs/diagnostics/scale_calib_exp1G_day_eventsoft_accept_highconf \
    > '${day_log}' 2>&1
"
...
echo "Launched scale_calib_exp1G_day_eventsoft on GPU ${GPU_SCALE_CALIB_DAY}, log=${day_log}"
