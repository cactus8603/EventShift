...
CUDA_VISIBLE_DEVICES="${GPU_SCALE_CALIB_DAY}" \
  nohup conda run --no-capture-output -n "${CONDA_ENV}" \
    ...
    --out-dir work_dirs/diagnostics/scale_calib_exp1A_day_accept_highconf \
    > "${log_dir}/scale-calib-exp1A-day_${stamp}.log" 2>&1 &
pid_day="$!"
...
CUDA_VISIBLE_DEVICES="${GPU_SCALE_CALIB_NIGHT}" \
  nohup conda run --no-capture-output -n "${CONDA_ENV}" \
    ...
    --device cuda:0 \
    --eval-cache-dir "${cache_root}" \
    --out-dir work_dirs/diagnostics/scale_calib_exp1B_night_accept_highconf \
    > "${log_dir}/scale-calib-exp1B-night_${stamp}.log" 2>&1 &
pid_night="$!"

echo "Launched scale-calib-exp1A Day on GPU ${GPU_SCALE_CALIB_DAY}: pid=${pid_day}, log=${log_dir}/scale-calib-exp1A-day_${stamp}.log"
echo "Launched scale-calib-exp1B Night on GPU ${GPU_SCALE_CALIB_NIGHT}: pid=${pid_night}, log=${log_dir}/scale-calib-exp1B-night_${stamp}.log"
