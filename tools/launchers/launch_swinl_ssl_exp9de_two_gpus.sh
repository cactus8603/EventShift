...
CUDA_VISIBLE_DEVICES="${GPU_EXP9E}" CONDA_ENV="${CONDA_ENV}" \
  nohup bash tools/run_swinl_ssl_currentbest_tta_segformer_agree_gap_focus_headonly.sh \
  > "${log_dir}/ssl-exp9e_${stamp}.log" 2>&1 &
pid_exp9e="$!"

echo "Launched ssl-exp9d on GPU ${GPU_EXP9D}: pid=${pid_exp9d}, log=${log_dir}/ssl-exp9d_${stamp}.log"
echo "Launched ssl-exp9e on GPU ${GPU_EXP9E}: pid=${pid_exp9e}, log=${log_dir}/ssl-exp9e_${stamp}.log"
