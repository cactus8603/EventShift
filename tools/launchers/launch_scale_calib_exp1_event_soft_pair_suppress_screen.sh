...
day_deny_pairs="vegetation->wall,vegetation->sidewalk,road->sidewalk,vegetation->road,traffic sign->pole,sky->pole,sky->vegetation,building->vegetation"
night_deny_pairs="building->vegetation,traffic sign->road,road->sidewalk,traffic sign->wall,fence->wall"

day_log="${log_dir}/scale-calib-exp1I-day-eventsoft-pairsuppress_${stamp}.log"
night_log="${log_dir}/scale-calib-exp1J-night-eventsoft-pairsuppress_${stamp}.log"

screen -dmS scale_calib_exp1I_day_eventsoft_pairsuppress bash -lc "
  cd '${ROOT}' &&
  export PYTHONNOUSERSITE=1 &&
  export PYTHONUNBUFFERED=1 &&
  CUDA_VISIBLE_DEVICES='${GPU_SCALE_CALIB_DAY}' conda run --no-capture-output -n '${CONDA_ENV}' \
    ...
    --flip \
    --device cuda:0 \
    --train-matrix-path work_dirs/diagnostics/scale_calib_exp1E_day_eventedge_accept_highconf/train_matrix.npz \
    --reuse-train-matrix \
    --eval-cache-dir '${cache_root}' \
    --reuse-eval-cache \
    --out-dir work_dirs/diagnostics/scale_calib_exp1I_day_eventsoft_pairsuppress \
    > '${day_log}' 2>&1
"

screen -dmS scale_calib_exp1J_night_eventsoft_pairsuppress bash -lc "
  cd '${ROOT}' &&
  export PYTHONNOUSERSITE=1 &&
  export PYTHONUNBUFFERED=1 &&
  CUDA_VISIBLE_DEVICES='${GPU_SCALE_CALIB_NIGHT}' conda run --no-capture-output -n '${CONDA_ENV}' \
    ...
    --flip \
    --device cuda:0 \
    --train-matrix-path work_dirs/diagnostics/scale_calib_exp1F_night_eventedge_accept_highconf/train_matrix.npz \
    --reuse-train-matrix \
    --eval-cache-dir '${cache_root}' \
    --reuse-eval-cache \
    --out-dir work_dirs/diagnostics/scale_calib_exp1J_night_eventsoft_pairsuppress \
    > '${night_log}' 2>&1
"

echo "Launched scale_calib_exp1I_day_eventsoft_pairsuppress on GPU ${GPU_SCALE_CALIB_DAY}, log=${day_log}"
echo "Launched scale_calib_exp1J_night_eventsoft_pairsuppress on GPU ${GPU_SCALE_CALIB_NIGHT}, log=${night_log}"
