...
CUDA_VISIBLE_DEVICES="${GPU_SCALE_CALIB_DAY}" \
  nohup conda run --no-capture-output -n "${CONDA_ENV}" \
    ...
    > "${log_dir}/scale-calib-exp1E-day-eventedge_${stamp}.log" 2>&1 &
pid_day="$!"
...
CUDA_VISIBLE_DEVICES="${GPU_SCALE_CALIB_NIGHT}" \
  nohup conda run --no-capture-output -n "${CONDA_ENV}" \
    ...
    > "${log_dir}/scale-calib-exp1F-night-eventedge_${stamp}.log" 2>&1 &
pid_night="$!"

echo "Launched scale-calib-exp1E Day event-edge on GPU ${GPU_SCALE_CALIB_DAY}: pid=${pid_day}, log=${log_dir}/scale-calib-exp1E-day-eventedge_${stamp}.log"
echo "Launched scale-calib-exp1F Night event-edge on GPU ${GPU_SCALE_CALIB_NIGHT}: pid=${pid_night}, log=${log_dir}/scale-calib-exp1F-night-eventedge_${stamp}.log"
