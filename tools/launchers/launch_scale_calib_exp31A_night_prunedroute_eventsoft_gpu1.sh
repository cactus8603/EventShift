...
if [ ! -f "${SOURCE_TRAIN_MATRIX}" ]; then
  ...
  echo "Wait for Exp30B to finish writing its matrix before launching Exp31A." >&2
  exit 1
fi
...
if [ ! -f "${SOURCE_EVAL_CACHE_DIR}/cosec_night_val_event/manifest.json" ]; then
  ...
  echo "Wait for Exp30B to finish eval-cache collection before launching Exp31A." >&2
  exit 1
fi

stamp="$(date +%Y%m%d_%H%M%S)"
session="scale_calib_exp31A_night_no_tsign_no_road_${stamp}"
...
out_dir="work_dirs/diagnostics/scale_calib_exp31A_night_no_tsign_no_road_eventsoft_full"
eval_cache_dir="work_dirs/diagnostics/scale_calib_exp31A_night_no_tsign_no_road_eventsoft_eval_cache"

screen -dmS "${session}" bash -lc "
  cd '${ROOT}' &&
  export PYTHONNOUSERSITE=1 &&
  export PYTHONUNBUFFERED=1 &&
  CUDA_VISIBLE_DEVICES='${GPU_SCALE_CALIB_EXP31A}' conda run --no-capture-output -n '${CONDA_ENV}' \
    ...
    --class-route 'sidewalk->highres768,wall->tta3,vegetation->tta3,sky->highres768,car->highres768' \
    --target-classes sidewalk,wall,vegetation,sky,car \
    --target-match anchor \
    --reuse-train-matrix \
...
echo "Launched ${session} on GPU ${GPU_SCALE_CALIB_EXP31A}"
echo "log=${log_path}"
echo "out=${out_dir}"
