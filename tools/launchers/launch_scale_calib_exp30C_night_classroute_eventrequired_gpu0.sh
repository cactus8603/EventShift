...
session="scale_calib_exp30C_night_eventrequired_${stamp}"
...
out_dir="work_dirs/diagnostics/scale_calib_exp30C_night_classroute_eventrequired_full"
eval_cache_dir="work_dirs/diagnostics/scale_calib_exp30C_night_classroute_eventrequired_eval_cache"

screen -dmS "${session}" bash -lc "
  cd '${ROOT}' &&
  export PYTHONNOUSERSITE=1 &&
  export PYTHONUNBUFFERED=1 &&
  CUDA_VISIBLE_DEVICES='${GPU_SCALE_CALIB_EXP30C}' conda run --no-capture-output -n '${CONDA_ENV}' \
...
echo "Launched ${session} on GPU ${GPU_SCALE_CALIB_EXP30C}"
echo "log=${log_path}"
echo "out=${out_dir}"
